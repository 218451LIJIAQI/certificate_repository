...
class ActivityService {
  ...
  Future<void> logActivity({
    ...
    try {
      ...
      await _firestore.collection(AppConfig.accessLogsCollection).add({
        ...
        'ipAddress': null, // Would be filled by backend function
        'userAgent': null, // Would be filled by backend function
      });
  ...
  Future<List<Map<String, dynamic>>> getFormattedRecentActivities({
    ...
    try {
      final activities = await getRecentActivities(userId: userId, limit: limit);
      ...
      return activities.map((activity) {
        ...
        return {
          ...
          'subtitle': details,
          'time': timestamp != null ? _formatRelativeTime(timestamp.toDate()) : 'Unknown time',
  ...
  Future<void> cleanupOldActivities({int retentionDays = 180}) async {
    try {
      ...
      await batch.commit();
      LoggerService.info('Cleaned up ${snapshot.docs.length} old activity logs');
    } catch (e) {
      ...
      for (final activity in activities) {
        ...
        if (timestamp != null) {
          ...
          if (dailyTrends.containsKey(dateKey)) {
            dailyTrends[dateKey]![action] = (dailyTrends[dateKey]![action] ?? 0) + 1;
          }
        }
      }
