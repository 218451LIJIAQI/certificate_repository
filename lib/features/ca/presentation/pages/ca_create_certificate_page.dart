...
class CACreateCertificatePage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CACreateCertificatePage> createState() => _CACreateCertificatePageState();
}

class _CACreateCertificatePageState extends ConsumerState<CACreateCertificatePage> {
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: AppTheme.spacingM),
                    _buildSummaryRow('Type', _getCertificateTypeDisplayName(_selectedType)),
                    _buildSummaryRow('Title', _titleController.text.isNotEmpty ? _titleController.text : 'Not specified'),
                    _buildSummaryRow('Recipient', _recipientNameController.text.isNotEmpty ? _recipientNameController.text : 'Not specified'),
                    _buildSummaryRow('Email', _recipientEmailController.text.isNotEmpty ? _recipientEmailController.text : 'Not specified'),
                    _buildSummaryRow('Issue Date', '${_issuedDate.day}/${_issuedDate.month}/${_issuedDate.year}'),
                  ],
                ),
              ),
            ),
  ...
  bool _validateRecipientInfo() {
    ...
    if (!RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$').hasMatch(_recipientEmailController.text)) {
  ...
  Future<void> _createCertificate() async {
    ...
    try {
      ...
      // Listen for success
      ref.listen<CertificateCreationState>(certificateCreationProvider, (previous, next) {
