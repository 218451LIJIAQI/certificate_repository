...
final caStatsProvider = StateNotifierProvider<CAStatsNotifier, AsyncValue<CAStats>>((ref) {
  return CAStatsNotifier(ref.read(caServiceProvider));
});

// Pending Documents for Review Provider
final pendingDocumentsProvider = FutureProvider<List<DocumentModel>>((ref) async {
...
final documentReviewProvider = StateNotifierProvider<DocumentReviewNotifier, DocumentReviewState>((ref) {
  return DocumentReviewNotifier(ref.read(caServiceProvider));
});

// CA Activity Log Provider
final caActivityProvider = StreamProvider<List<CAActivity>>((ref) {
  ...
  return FirebaseFirestore.instance
      ...
      .snapshots()
      .map((snapshot) => snapshot.docs
          .map((doc) => CAActivity.fromFirestore(doc))
          .toList());
});

// CA Settings Provider
final caSettingsProvider = StateNotifierProvider<CASettingsNotifier, CASettingsState>((ref) {
...
class CAStatsNotifier extends StateNotifier<AsyncValue<CAStats>> {
  ...
  Future<void> loadStats() async {
    try {
      ...
      LoggerService.error('Failed to load CA stats', error: error, stackTrace: stackTrace);
...
class CertificateCreationNotifier extends StateNotifier<CertificateCreationState> {
  CertificateCreationNotifier(this._caService) : super(const CertificateCreationState());
...
class CASettingsNotifier extends StateNotifier<CASettingsState> {
  ...
  Future<void> loadSettings() async {
    try {
      ...
      LoggerService.error('Failed to load CA settings', error: error, stackTrace: stackTrace);
      state = state.copyWith(
        isLoading: false,
        error: error.toString(),
      );
...
class CASettingsModel {
  ...
  factory CASettingsModel.fromFirestore(DocumentSnapshot doc) {
    ...
    return CASettingsModel(
      ...
      allowedFileTypes: List<String>.from(data['allowedFileTypes'] ?? ['pdf', 'doc', 'docx', 'jpg', 'png']),
