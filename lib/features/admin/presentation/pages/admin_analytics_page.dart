...
final systemOverviewReportProvider = FutureProvider<Map<String, dynamic>>((ref) {
...
class _AdminAnalyticsPageState extends ConsumerState<AdminAnalyticsPage>
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(
              height: 200,
              child: _buildActivityChart(activity['dailyActivity'] as Map<String, dynamic>? ?? {}),
            ),
          ],
        ),
      ),
  ...
  Widget _buildRegistrationTrendsCard(Map<String, dynamic> monthlyRegistrations) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(AppTheme.spacingM),
        child: Column(
  ...
  Widget _buildCAPerformanceCard(Map<String, dynamic> report) {
    final caPerformance = report['caPerformance'] as Map<String, dynamic>? ?? {};
        ...
        showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            content: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Your report has been generated in multiple formats:'),
                const SizedBox(height: 16),
                _buildDownloadOption('PDF Report', pdfUrl, Icons.picture_as_pdf),
                _buildDownloadOption('Excel Report', excelUrl, Icons.table_chart),
  ...
  Map<String, dynamic> _formatReportData(Map<String, dynamic> systemStats, Map<String, dynamic> caStats) {
    return {
  ...
  String _convertToCsv(Map<String, dynamic> data) {
    ...
    buffer.writeln('System,Total Users,${system['users']['totalUsers']},${data['exportedAt']}');
    buffer.writeln('System,Total Certificates,${system['certificates']['totalCertificates']},${data['exportedAt']}');
    buffer.writeln('System,Total Documents,${system['documents']['totalDocuments']},${data['exportedAt']}');
  ...
  Future<void> _saveToFile(String content, String filename) async {
    try {
      ...
      final result = await ref.read(reportExportServiceProvider).saveTextFile(
        content: content,
        filename: filename,
      );
  ...
  void _exportAnalytics() async {
    try {
      ...
      await _saveToFile(csvData, 'analytics_export_${DateTime.now().millisecondsSinceEpoch}.csv');
