...
class CertificateDetailPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CertificateDetailPage> createState() => _CertificateDetailPageState();
}
...
class _CertificateDetailPageState extends ConsumerState<CertificateDetailPage>
  ...
  Widget build(BuildContext context) {
    final certificateAsync = ref.watch(certificateDetailProvider(widget.certificateId));
  ...
  Widget _buildCertificateDetail(CertificateModel certificate, UserModel? user) {
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: _getTypeColor(certificate.type).withValues(alpha: 0.1),
          ...
          _buildInfoSection(
            ...
            children: [
              ...
              if (certificate.completedAt != null)
                _buildInfoRow('Completion Date', _formatDate(certificate.completedAt!)),
              if (certificate.expiresAt != null)
                _buildInfoRow('Expiry Date', _formatDate(certificate.expiresAt!)),
            ],
          ),
    ...
    return SingleChildScrollView(
        ...
        child: Column(
          ...
          _buildInfoSection(
            ...
            children: [
              _buildInfoRow('Verification ID', certificate.verificationId),
              _buildInfoRow('Digital Signature', certificate.digitalSignature.isNotEmpty ? 'Present' : 'Not Available'),
              _buildInfoRow('Verification Level', certificate.verificationLevel.name.toUpperCase()),
              _buildInfoRow('Is Verified', certificate.isVerified ? 'Yes' : 'No'),
              _buildInfoRow('Access Count', certificate.accessCount.toString()),
              _buildInfoRow('Verification Count', certificate.verificationCount.toString()),
            ],
          ),
          const SizedBox(height: AppTheme.spacingL),
          _buildInfoSection(
            ...
            children: [
              ...
              _buildInfoRow('Last Updated', _formatDateTime(certificate.updatedAt)),
              if (certificate.lastAccessedAt != null)
                _buildInfoRow('Last Accessed', _formatDateTime(certificate.lastAccessedAt!)),
        ...
        child: Column(
          ...
          _buildInfoSection(
            ...
            children: [
              ...
              _buildInfoRow('Verifications', certificate.verificationCount.toString()),
          ...
          if (certificate.shareTokens.isNotEmpty) ...[
            ...
            _buildInfoSection(
              title: 'Share Tokens',
              icon: Icons.share,
              children: certificate.shareTokens.map((token) => 
                _buildShareTokenItem(token)
              ).toList(),
            ),
          ],
        ],
  ...
  Widget _buildShareTokenItem(ShareToken token) {
    return Container(
      ...
      padding: const EdgeInsets.all(AppTheme.spacingM),
      decoration: BoxDecoration(
        color: token.isValid ? AppTheme.successColor.withValues(alpha: 0.1) : AppTheme.errorColor.withValues(alpha: 0.1),
        borderRadius: AppTheme.smallRadius,
        border: Border.all(
          color: token.isValid ? AppTheme.successColor.withValues(alpha: 0.3) : AppTheme.errorColor.withValues(alpha: 0.3),
        ),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
          ...
          Row(
            children: [
              Icon(
                ...
                color: token.isValid ? AppTheme.successColor : AppTheme.errorColor,
                size: 16,
              ),
  ...
  bool _canApproveCertificate(CertificateModel certificate, UserModel? user) {
    if (user == null || !user.canApproveCertificates) return false;
    
    return certificate.status == CertificateStatus.pending &&
           certificate.approvalSteps.any((step) => 
...
''';
  ...
  void _downloadCertificate(CertificateModel certificate) async {
    try {
      ...
      final downloadUrl = await pdfService.uploadPdfToStorage(pdfBytes, certificate);
      ...
      if (Platform.isAndroid || Platform.isIOS) {
        ...
        final localPath = await pdfService.downloadPdfLocally(pdfBytes, fileName);
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                Expanded(
                  child: Text('Certificate "${certificate.title}" downloaded successfully'),
                ),
              ],
            ),
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                const SizedBox(width: 8),
                Expanded(child: Text('Failed to download certificate: ${e.toString()}')),
              ],
            ),
  ...
  void _handleMenuAction(String action, CertificateModel certificate, UserModel? user) {
    switch (action) {
      case 'approve':
